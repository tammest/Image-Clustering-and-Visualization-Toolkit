...
# Function to plot individual cluster tile positions
def individual_cluster_plot(tile_df, clusters, colors, sample_name):
    h = tile_df['h'].values
    w = tile_df['w'].values

    num_clusters = len(colors)
    
    # Create a plot for each individual cluster
    for cluster_number in range(num_clusters):
        # Get the tile indices for the current cluster
        cluster_indices = np.where(clusters == cluster_number)[0]
        h_cluster = h[cluster_indices]
        w_cluster = w[cluster_indices]
        ...
        plt.title(f'Tile Positions for Cluster {cluster_number} - {sample_name}')
        plt.xlabel('w (width)')
        plt.ylabel('h (height)')
        plt.gca().invert_yaxis()  # Invert y-axis to match image coordinates
        plt.legend()
        ...
        # Save the plot as a PDF
        plt.savefig(f'cluster_{cluster_number}_tile_position_{sample_name}.pdf')  # Save as PDF with the sample name
        plt.close()  # Close the plot to free up memory

    print(f"Individual cluster tile position plots saved for {sample_name}.")
...
def main(root_path, sample_names, tile_size=128):
    for sample_name in sample_names:
        ...
        # Plot the tile positions for each cluster individually
        individual_cluster_plot(tile_df, clusters, colors, sample_name)  # This is the new function added here
        
        # Save tile positions from selected clusters (e.g., clusters 0 and 1)
        save_tiles_from_clusters(tile_df, clusters, selected_clusters=[0, 1], output_filename=f"{sample_name}_selected_tiles.csv")
        
        # Plot tile count per cluster
        plot_tile_count_per_cluster(clusters, n_clusters=30, sample_name=sample_name)

        # Generate a combined correlation plot for all channels
        plot_channel_correlation(tile_df, data, channels, tile_size, sample_name)
